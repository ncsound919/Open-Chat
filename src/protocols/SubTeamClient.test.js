...
import { subTeamStream, subTeamHealthCheck } from "./SubTeamClient.js";
...
describe("subTeamStream", () => {
  it("streams content chunks, calls onChunk per chunk, and returns the full text", async () => {
    ...
    const chunks = [];
    const result = await subTeamStream(
      "127.0.0.1",
      8642,
      "tok",
      [{ role: "user", content: "design a cpu" }],
      (c) => chunks.push(c)
    );
    ...
    expect(body.model).toBe("subteam-agent");
    expect(body.stream).toBe(true);
    expect(body.messages).toEqual([{ role: "user", content: "design a cpu" }]);
    expect(body.metadata).toEqual({ agent_type: "cpu_design", draymond_registered: true });
  });

  it("omits the Authorization header when no token is provided", async () => {
    fetchMock.mockResolvedValueOnce(jsonOk({}, sseBody(["ok"])));
    const result = await subTeamStream("localhost", 8642, "", [], vi.fn());
  ...
  it("throws on an HTTP error status", async () => {
    ...
    await expect(
      subTeamStream("127.0.0.1", 8642, "", [], vi.fn())
...
describe("subTeamHealthCheck", () => {
  it("returns true when the health endpoint is ok", async () => {
    ...
    const ok = await subTeamHealthCheck("127.0.0.1", 8642, "tok", 500);
    expect(ok).toBe(true);
    const [url, init] = fetchMock.mock.calls[0];
  ...
  it("returns false when the health endpoint reports a non-ok status", async () => {
    ...
    expect(await subTeamHealthCheck("127.0.0.1", 8642, "")).toBe(false);
  });
